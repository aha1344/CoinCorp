...
const WelcomeScreen = () => {
  ...
  const handleContinue = async () => {
    ...
    if (!isValidLebanesePhoneNumber(phoneNumber)) {
      alert("Please enter a valid Lebanese phone number (8 digits)");
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        "http://192.168.1.102:3000/check-phone-number",
        {
          method: "POST",
          headers: {
  ...
  // Function to validate Lebanese phone number format
  function isValidLebanesePhoneNumber(phone_number) {
    // Lebanese phone numbers should have 8 digits
    const eightDigitNumberRegex = /^\d{8}$/;
    return eightDigitNumberRegex.test(phone_number);
  }
